...
static int del_recv_keys_1(int sk)
{
...
static int set_send_keys_2(int sk)
{
	struct sctp_dtls_keys *key;
	uint8_t buf[128] = {};
...
static int set_recv_keys_2(int sk)
{
	struct sctp_dtls_keys *key;
	uint8_t buf[128] = {};
...
static int recv_with_flags(int csk, char *buf, unsigned int len, int *flags)
{
	struct msghdr msg;
	struct iovec iov;
	int ret;
...
static int client_fake_dtls_handshake(int csk)
{
	int ret, flags = 0;
	char buf[128];
	...
	ret = recv_with_flags(csk, buf, sizeof(buf), &flags);
	if (ret < 0) {
		printf("failed to recv msg\n");
		return -1;
	}
	if (!(flags & MSG_PROTECTED)) {
		printf("msg not encypted\n");
		return -1;
	}
	...
	if (set_enforce_protection(csk))
		return -1;
	printf("DTLS Handshake\n");
	return 0;
}

static int client_say_hi(int csk)
{
	int ret, flags = 0;
	char buf[128];
...
static int client_fake_dtls_rekey(int csk)
{
	int ret, flags = 0;
	char buf[128];

	strcpy(buf, "dtls_rekey");
	ret = send(csk, buf, strlen(buf) + 1, 0);
	if (ret < 0) {
		printf("failed to send msg\n");
		return -1;
	...
	if (set_send_keys_2(csk))
		return -1;
	printf("DTLS Rekey\n");
	return 0;
}

static int client_exit(int csk)
{
	char buf[128];
	int ret;
...
static int client_use_old_key(int csk)
{
	if (set_send_keys_1(csk))
		return -1;
	printf("Use Old Key\n");
...
static int client_use_new_key(int csk)
{
	if (set_send_keys_2(csk))
		return -1;
	printf("Use New Key\n");
...
static int client_del_old_key(int csk)
{
	if (del_recv_keys_1(csk))
		return -1;
	printf("Del Old Key\n");
	return 0;
}

static int client_fake_dtls_crash(int csk)
{
	struct linger l;

	system("iptables -A OUTPUT -p sctp -j DROP");
	system("ip6tables -A OUTPUT -p sctp -j DROP");
	l.l_onoff = 1;
	l.l_linger = 0;
	if (setsockopt(csk, SOL_SOCKET, SO_LINGER, &l, sizeof(l)) < 0) {
		printf("failed to setsockopt SO_LINGER\n");
		system("iptables -D OUTPUT -p sctp -j DROP");
		system("ip6tables -D OUTPUT -p sctp -j DROP");
		return -1;
	}
...
static int do_client(int argc, char *argv[])
{
	...
	ret = getsockname(csk, (struct sockaddr *)&local, &len);
	if (ret) {
		printf("failed to getsockname\n");
		return -1;
	}

	if (client_fake_dtls_handshake(csk))
		return -1;

	if (client_say_hi(csk))
		return -1;

	if (client_fake_dtls_rekey(csk))
		return -1;

	if (client_say_hi(csk))
		return -1;

	if (client_use_old_key(csk))
		return -1;

	if (client_say_hi(csk))
		return -1;

	if (client_use_new_key(csk))
		return -1;
	...
	if (client_del_old_key(csk))
		return -1;

	if (client_fake_dtls_crash(csk))
		return -1;

	/* Protected SCTP Restart */
	csk = socket(remote.ss_family, SOCK_STREAM, IPPROTO_SCTP);
	if (csk < 0) {
		printf("failed to create socket\n");
		return -1;
	...
	ret = bind(csk, (struct sockaddr *)&local, len);
	if (ret < 0) {
...
static int server_fake_dtls_handshake(int csk, char *buf)
{
	int ret;
...
static int server_say_hi(int csk, char *buf)
{
	int ret;
...
static int server_fake_dtls_rekey(int csk, char *buf)
{
	int ret;

	if (set_recv_keys_2(csk))
		return -1;
...
static int do_server(int argc, char *argv[])
{
	...
	while (1) {
		...
		if (!strcmp(buf, "dtls_handshake")) {
			if (server_fake_dtls_handshake(csk, buf))
				return -1;
		} else if (!strcmp(buf, "dtls_rekey")) {
			if (server_fake_dtls_rekey(csk, buf))
				return -1;
		} else if (!strcmp(buf, "hi")) {
			if (server_say_hi(csk, buf))
				return -1;
		} else {
			break;
		}
